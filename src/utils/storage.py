...
directory = os.getenv('LOCAL_PATH', 'static/')
if stage == 'prod':
...
def get_config(config_name):
    ...
    """
    if stage == 'prod':
        json_file = get_file_from_bucket(directory + '/layouts/' + config_name + '.json')
        data = json.loads(json_file)
        return data
    elif stage == 'dev':
        print("Running in dev environment, loading config from local file system")
        with open(directory + '/layouts/' + str(config_name) + '.json') as json_file:
...
def get_image(image_name):
    ...
    """
    if stage == 'prod':
        image_blob = bucket.get_blob(directory + '/images/' + image_name).download_as_string()
        img_bytes = io.BytesIO(image_blob)
        img = Image.open(img_bytes)
    elif stage == 'dev':
        print("Running in dev environment, loading image from local file system")
        img = Image.open(directory + '/images/' + image_name)
...
def get_font(font_name, font_size):
    ...
    """
    if stage == 'prod':
        font_blob = get_file_from_bucket(directory + '/fonts/' + font_name + '.ttf')
        ...
        try:
            return ImageFont.truetype(directory + '/fonts/' + font_name + '.ttf', font_size)
        except OSError:
